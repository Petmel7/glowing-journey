<section class="master">
    <div class="container">

        <h1 class="master-title">Master the <span class="master-title-language">Language</span> with Our English Package!</h1>

        <div class="checkbox">

            <div>
                <h2 class="master-title-power">Power of English</h2>

                <!-- <div class="master-block">
                    <label class="master-checkbox" for="checkbox1">
                        <svg class="custom-checkbox" viewBox="0 0 24 24">
                            <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                        </svg>
                        <input class="master-checkbox-input" type="checkbox" id="checkbox1" name="checkbox1">
                        <p class="master-checkbox-text">Dive into captivating lessons designed to stimulate your curiosity.</p>
                    </label>
                    <label class="master-checkbox" for="checkbox2">
                        <svg class="custom-checkbox" viewBox="0 0 24 24">
                            <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                        </svg>
                        <input class="master-checkbox-input" type="checkbox" id="checkbox2" name="checkbox2">
                        <p class="master-checkbox-text">Learn in a nurturing setting where every student's progress matters.</p>
                    </label>
                    <label class="master-checkbox" for="checkbox3">
                        <svg class="custom-checkbox" viewBox="0 0 24 24">
                            <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                        </svg>
                        <input class="master-checkbox-input" type="checkbox" id="checkbox3" name="checkbox3">
                        <p class="master-checkbox-text">Experience the satisfaction of real progress as your language skills and confidence grow with every session.</p>
                    </label>
                </div> -->

                <div class="master-block">

                    <div class="checkbox-container">
                        <input class="master-checkbox-input" type="checkbox" id="checkbox1" name="">
                        <label class="master-checkbox" for="checkbox1">
                            <svg class="custom-checkbox" viewBox="0 0 24 24">
                                <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                            </svg>
                            <p class="master-checkbox-text">Dive into captivating lessons designed to stimulate your curiosity.</p>
                        </label>
                    </div>

                    <div class="checkbox-container">
                        <input class="master-checkbox-input" type="checkbox" id="checkbox2" name="">
                        <label class="master-checkbox" for="checkbox2">
                            <svg class="custom-checkbox" viewBox="0 0 24 24">
                                <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                            </svg>
                            <p class="master-checkbox-text">Learn in a nurturing setting where every student's progress matters.</p>
                        </label>
                    </div>

                    <div class="checkbox-container">
                        <input class="master-checkbox-input" type="checkbox" id="checkbox3" name="">
                        <label class="master-checkbox" for="checkbox3">
                            <svg class="custom-checkbox" viewBox="0 0 24 24">
                                <use href="./images.svg/symbol-defs.svg#icon-check"></use>
                            </svg>
                            <p class="master-checkbox-text">Experience the satisfaction of real progress as your language skills and confidence grow with every session.</p>
                        </label>
                    </div>
                </div>

                <button class="master-button" type="button">Leave an application</button>

            </div>

            <div class="master-background-image"></div>
        </div>
    </div>
</section>